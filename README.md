...
- 🎯 Entry points to execute your program using `python -m <python_inventory_api>` or `$ python_inventory_api` with basic CLI argument parsing.
...
---
# python_inventory_api

[![codecov](https://codecov.io/gh/nivecher/python-inventory-api/branch/main/graph/badge.svg?token=python-inventory-api_token_here)](https://codecov.io/gh/nivecher/python-inventory-api)
[![CI](https://github.com/nivecher/python-inventory-api/actions/workflows/main.yml/badge.svg)](https://github.com/nivecher/python-inventory-api/actions/workflows/main.yml)

Awesome python_inventory_api created by nivecher

## Install

from source
```bash
git clone https://github.com/nivecher/python-inventory-api python_inventory_api
cd python_inventory_api
make install
```
...
```bash
pip install python_inventory_api
```

## Executing

```bash
$ python_inventory_api run --port 8080
```

or

```bash
python -m python_inventory_api run --port 8080
```

or

```bash
$ uvicorn python_inventory_api:app
```

## CLI

```bash
❯ python_inventory_api --help
Usage: python_inventory_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ python_inventory_api create-user --help
Usage: python_inventory_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
python_inventory_api create-user admin admin --superuser
```
...
Run with `python_inventory_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `python_inventory_api create-user`.
...
-----------------------------------------------------
python_inventory_api/__init__.py              4      0   100%
python_inventory_api/app.py                  16      1    94%
python_inventory_api/cli.py                  21      0   100%
python_inventory_api/config.py                5      0   100%
python_inventory_api/db.py                   10      0   100%
python_inventory_api/models/__init__.py       0      0   100%
python_inventory_api/models/content.py       47      1    98%
python_inventory_api/routes/__init__.py      11      0   100%
python_inventory_api/routes/content.py       52     25    52%
python_inventory_api/routes/security.py      15      1    93%
python_inventory_api/routes/user.py          52     26    50%
python_inventory_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in python_inventory_api/default.toml.
